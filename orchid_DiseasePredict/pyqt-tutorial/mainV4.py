...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def update_current_thermal_plane_II(self):
        ...
        for fs in range(len(self.fixedsensors)):
            self.temp = []
            self.humid = []
            self.light = []
            self.timestamp = []
            for n in range(len(getjsons['data'][-576:])):
                self.temp.append(getjsons['data'][n-576]['temperature'])
                self.humid.append(getjsons['data'][n-576]['wetness'])
                self.light.append(getjsons['data'][n-576]['par']/55)
                self.timestamp.append(getjsons['data'][n-576]['time'])
        # -------------------------------------------------------------------------------------------------
