...
# -----------------------------------------------------
# -----------------------函式宣告-----------------------
def linear_interpolation(rawdata):
...
for number in range(1):
    # 讀取感測器資料
    rawdata = pd.read_csv("sensorNode{}.csv".format(number+1)) 
    # 轉為矩陣
    rawdata_array = np.array(rawdata)
    total_LossNumber = 0
    # 另存名為 paddeddata_array 作為線性插植後的感測器資料
    ...
    with open("PaddedMovingAverage{}_data{}.csv".format(ma, number+1), 'w', newline='') as thecsvfile:
        ...
        #     writer.writerow(row)
        print("len(paddeddata_array):{}".format(len(paddeddata_array)))
        for n in range(len(paddeddata_array)-(ma-1)):
            humid_moving_sum = 0
            temp_moving_sum = 0
            par_moving_sum = 0
            for k in range(ma):
                ...
                print(k)
                print(n + k)
                humid_moving_sum += paddeddata_array[n+k][1]
                temp_moving_sum += paddeddata_array[n+k][2]
                par_moving_sum += paddeddata_array[n+k][3]
